...
void helloWorld()
{
  ...
  do
  {
    ...
    display.getTextBounds("StartLabs", 0, 0, &tbx, &tby, &tbw, &tbh);
    uint16_t x = ((display.width() - tbw) / 2) - tbx;
    uint16_t y = ((display.height()) / 2) - tby;

    display.setCursor(x, y);
    display.print("StartLabs");

    display.getTextBounds("Cuenca", 0, 0, &tbx, &tby, &tbw, &tbh);
    x = ((display.width() - tbw) / 2) - tbx;
    y = ((display.height()) / 2) - tby;
    
    display.setTextColor(GxEPD_RED);
    display.setCursor(x, y+tbh+2);
    display.print("Cuenca");
  }
